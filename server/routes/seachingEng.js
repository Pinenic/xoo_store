...
const countLimit = (await supabase.from("products").select("*").like("name",`%${seaching}%`)).data.length;
const limit = req.query.limit || countLimit; // Default limit to the number of matching products
  try {
    const { data } = await supabase.from("products").select("*").like("name",`%${seaching}%`).limit(limit);
